...
export function Chat({ selectedChat }: { selectedChat: chat }) {
  ...
  useEffect(() => {
    // Desplazarse al último mensaje cuando los mensajes cambian
    messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [messages]); // Asegúrate de que esto se ejecute cada vez que `messages` cambie
  ...
  const handleDelete = async (messageId: string) => {
    ...
    // Actualiza los mensajes después de eliminar
    setMessages((prevMessages) => prevMessages.filter(x => x.id !== messageId));
  };
      ...
      <div className="flex-1 p-4 bg-[#EFEFEF] overflow-y-auto">
        {isLoading ? (
          ...
          messageFilter().map((message, i) => (
            ...
            >
              <div
                className={`rounded-xl p-3 max-w-sm shadow-md ${message.receiverId !== id ? 'bg-[#DCF8C6] text-left' : 'bg-white text-right'}`}
              >
        ...
        {/* Referencia para el desplazamiento al último mensaje */}
  ...
  <input
    className="flex-1 bg-white rounded-full py-2 px-4 text-[#303030] focus:outline-none focus:ring-2 focus:ring-[#008069] placeholder-gray-500 transition duration-200"
    value={message}
  ...
  <label
    htmlFor="image-input"
    className="p-2 text-[#606060] hover:text-gray-400 cursor-pointer transition duration-200"
  ...
  <button
    ...
    className={`p-2 text-white rounded-full focus:outline-none transition duration-200 ${
      message.length < 1 ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#008069] hover:bg-[#007B63]'
    }`}
