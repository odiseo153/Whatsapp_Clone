...
export function Conversaciones({ setSelectedChat }: { setSelectedChat: (chat: chat) => void }) {
  ...
  const [width] = useState(25); // Estado para controlar el ancho del componente
  ...
  return (
    <div style={{ width: `${width}%` }} className="h-screen bg-[#111B21] border-r">
      <div className="p-4 bg-[#202C33] flex justify-between items-center" style={{ height: '40px' }}>
        ...
        {isOpen && (
          <MessageModal
            ...
            onClose={() => setOpenModal(false)} // Se cambia a función para cerrar correctamente el modal
          />
        )}
      ...
      <div className="p-2">
        <button
          onClick={() => setOpenModal(true)} // Cambiado a true para abrir el modal
          className="p-2 w-9 text-white bg-green-500 rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#E43F5A]"
          title="Crear chat"
        >
